...
pub async fn archive_to_s3(
    ...
    for directory in directories_list {
        ...
        let file_size = get_file_size(&output_file_path.clone()).await;
        let part_size = calculate_part_size(file_size).await;

        println!("File Size: {}", file_size);
        println!("Part Size: {}", part_size);

        if let Ok(metadata) = fs::metadata(output_file_path.clone()).await {
...
mod archivist_test {
    ...
    async fn test_archive_to_s3() {
        ...
        let archivist = archive_to_s3(directory_name, "rusty-bucket-2834", 8000000).await;
